...
configurationFile    = input_dir  + "ConfigurationV5.xlsx"
...
# DataFile and Data Frames + Loaders
...
sitadel17PL : pd.DataFrame = None
sitadelMeta : pd.DataFrame = None
sitadel13PL : pd.DataFrame = None
...
def load_sitadel(sitadel1316_file:  str = data_dir + sitadel1316File,
...
sitadel_locaux_17PL : pd.DataFrame = None
sitadel_locaux_Meta : pd.DataFrame = None
sitadel_locaux_13PL : pd.DataFrame = None
...
def load_sitadel_locaux(sitadelLocaux1316_file:  str = sitadelLocaux1316File,
                        sitadelLocaux17PL_file:  str = sitadelLocaux17PLFile,
    ...
    if (sitadel_locaux_1316 is None) or (sitadel_locaux_17PL is None)  or (sitadel_locaux_Meta is None):
        ...
        Term.print_blue("Lecture Sitadel Locaux 2017-2022 : " + sitadelLocaux17PL_file + " ...")
        sitadel_locaux_17PL = pd.read_csv(sitadelLocaux17PL_file, delimiter=';', index_col=4, encoding='latin-1', dtype={"DEP": str, "COMM": str, "Etat_DAU": str, "DPC_AUT": str, "ADR_LOCALITE_TER" : str, "ADR_CODPOST_TER" : str, "NATURE_PROJET" : str, "ZONE_OP": str, "I_EXTENSION": str, "I_SURELEVATION": str, "I_NIVSUPP": str, "SUPERFICIE_TERRAIN": float, "SURF_HAB_AVANT": float, "AN_DEPOT": str, "DPC_PREM" : str})
        sitadel_locaux_17PL["Parcelles"] = sitadel_locaux_17PL['sec_cadastre1'].map(str) + sitadel_locaux_17PL['num_cadastre1'].map(str)+" " + \
                                           sitadel_locaux_17PL['sec_cadastre2'].map(str) + sitadel_locaux_17PL['num_cadastre2'].map(str)+" " + \
                                           sitadel_locaux_17PL['sec_cadastre3'].map(str) + sitadel_locaux_17PL['num_cadastre3'].map(str)
        sitadel_locaux_17PL["NATURE_PROJET"]     = sitadel_locaux_17PL["NATURE_PROJET_DECLAREE"]
        sitadel_locaux_13PL = pd.concat([sitadel_locaux_1316, sitadel_locaux_17PL])
        Term.print_blue("Lecture Meta Locaux Sitadel : " + sitadelLocaux_meta_file + " ...")
        xls = pd.ExcelFile(sitadelLocaux_meta_file)
        sitadel_locaux_Meta = pd.read_excel(xls, 'Variables_Locaux', index_col=0)
    return sitadel_locaux_1316, sitadel_locaux_17PL, sitadel_locaux_Meta, sitadel_locaux_13PL
...
sitadel1019 : pd.DataFrame = None


def load_logements_paca(sitadel1019SourceFile:  str = sitadel1019SourceFile):
    global sitadel1019
    if (sitadel1019 is None) :
        Term.print_blue("Lecture Logements Paca 2010-2019 : " + sitadel1019SourceFile + " ...")
...
evolution0813 : pd.DataFrame = None
evolution1318 : pd.DataFrame = None
evolution1821 : pd.DataFrame = None
...
def load_evolution(evolution_file: str = evolutionFile):
    ...
    if (evolution0813 is None) or (evolution1318 is None)  or (evolution1821 is None):
        Term.print_blue("Lecture Evolution Dept : " + evolution_file + " ...")
...
evolution22SourcePage = "https://www.insee.fr/fr/statistiques/1893198"
evolution22SourceFile = "https://www.insee.fr/fr/statistiques/fichier/1893198/evolution-population-dep-2009-2022.xlsx"
evolution22File = data_dir + "evolution-population-dep-2009-2022.xlsx"
evolution220914 : pd.DataFrame = None
evolution221419 : pd.DataFrame = None
evolution221922 : pd.DataFrame = None
global_context["URL_SOURCE_EVOLUTION22"] = evolution22SourcePage

# Variation relative annuelle 2009-2022 (en %)
# Département, Estimations de population au 1er janvier 2022, Totale Due au solde naturel, Due au solde apparent des entrées et des sorties
# Ain	662,244	    0.7%	0.3%	0.4%
# Ain	662,244	    0.7%	0.3%	0.4%


def load_evolution22(evolution22_file: str = evolution22File):
    global evolution220914, evolution221419, evolution221922
    if (evolution220914 is None) or (evolution221419 is None)  or (evolution221419 is None):
        Term.print_blue("Lecture Evolution Dept : " + evolution22_file + " ...")
        xls = pd.ExcelFile(evolution22_file)
        evolution220914 = pd.read_excel(xls, '2009-2014', index_col=0, dtype={"Unnamed: 0": str})
        evolution221419 = pd.read_excel(xls, '2014-2019', index_col=0, dtype={"Unnamed: 0": str})
        evolution221922 = pd.read_excel(xls, '2019-2022', index_col=0, dtype={"Unnamed: 0": str})
    return evolution220914, evolution221419, evolution221922
...
projectionsDPT : pd.DataFrame = None
projectionsREG : pd.DataFrame = None
...
def load_projections(projections_file: str = projectionsFile):
    ...
    if (projectionsREG is None) or (projectionsREG is None):
        Term.print_blue("Lecture Projections Dept/Reg 2013-2050 : " + projections_file + " ...")
...
projectionsDPT_2022 : pd.DataFrame = None
projectionsREG_2022 : pd.DataFrame = None
...
def load_projections_2022(projections_file_2022: str = projectionsFile_2022):
    ...
    if (projectionsREG_2022 is None) or (projectionsREG_2022 is None):
        Term.print_blue("Lecture Projections Dept/Reg 2022-2070 : " + projections_file_2022 + " ...")
...
projectionsPaca : pd.DataFrame = None
...
def load_projections_paca(projections_paca_file: str = projectionsPacaFile):
    ...
    if (projectionsPaca is None) :
        Term.print_blue("Lecture Projections Paca : " + projections_paca_file + " ...")
...
departements     : pd.DataFrame = None
...
def load_departements(dpt_file: str = departementsFile):
    ...
    if (departements is None):
        Term.print_blue("Lecture Departements : " + dpt_file + " ...")
...
interCoSourcePage = "https://www.insee.fr/fr/information/2510634"
interCoSourceFile = "https://www.insee.fr/fr/statistiques/fichier/2510634/Intercommunalite_Metropole_au_01-01-2022.zip"
intercoFile = data_dir + "Intercommunalite_Metropole_au_01-01-2022.xlsx"

intercoDossier : pd.DataFrame = None
intercoEPCI    : pd.DataFrame = None
...
def load_interco(interco_file: str = intercoFile):
    ...
    if (intercoDossier is None) or (intercoEPCI is None):
        Term.print_blue("Lecture Donnees Intercommunalite : " + interco_file + " ...")
...
SCOT_DATA = {}
COMMUNES_HORS_PACA : pd.DataFrame = None
EPCI_SCOT          : pd.DataFrame = None
GROUPEMENTS        : pd.DataFrame = None


def load_scot_data(p_SCoT_File: str = SCoT_File):
    global SCOT_DATA, COMMUNES_HORS_PACA, EPCI_SCOT, GROUPEMENTS
    if (SCOT_DATA is None or SCOT_DATA == {}):
        SCOT_DATA = {}
        Term.print_blue("Lecture Donnees SCoT : " + p_SCoT_File + " ...")
        xls = pd.ExcelFile(p_SCoT_File)
        COMMUNES_HORS_PACA = pd.read_excel(xls, 'Communes hors PACA', dtype={"EPCI": str, "LIBEPCI": str,  "Commune": str,  "CODGEO": str})
    ...
    for groupe in SCOT_DATA["GROUPEMENTS"].keys():
        ...
        for territoire in str(territoires).split(',') :
            ...
            if (communes is None) or (len(communes) == 0) :
                Term.print_error("Territoire inconnu : " + territoire)
                continue
            ...
            SCOT_DATA["GROUPES_COMMUNES"][zone]["DEPT"] = c_dept
            SCOT_DATA["GROUPES_COMMUNES"][zone]["REG"]  = c_reg
    Files.save_file(to_json(SCOT_DATA,  indent=4), output_dir + os.sep + "a-scot.json")
    return SCOT_DATA
...
codesPostaux : pd.DataFrame = None
global_context["URL_SOURCE_CODES_POSTAUX"] = codesPostauxSourcePage

# Code_commune_INSEE;Nom_commune;Code_postal;Ligne_5;Libelle_d_acheminement;coordonnees_gps (lat,long)
...
def load_codes():
    ...
    if (codesPostaux is None) :
        Files.downloadFile(codesPostauxSourceFile, codesPostauxFile)
        Term.print_blue("Lecture Codes Postaux Donnees Communes : " + codesPostauxFile + " ...")
...
sru2017 : pd.DataFrame = None
sru2020 : pd.DataFrame = None
...
def load_sru(sru_file: str = sruFile):
    ...
    if (sru2017 is None) or (sru2020 is None):
        Term.print_blue("Lecture Carences SRU PACA : " + sru_file + " ...")
...
def load_communes(meta_dossier_file: str = metaDossierFile, dossier_complet_file: str = dossierCompletFile):
    ...
    if (metaDossier is None) or (dossierComplet is None):
        Files.downloadFile(metaDossierSourceFile, dossierCompletFile, is_zipped=True, zipped_file="dossier_complet.csv")
        Term.print_blue("Lecture Meta Donnees Communes : " + metaDossierFile + " ...")
        metaDossier = pd.read_csv(meta_dossier_file, delimiter=';', index_col=0)
        Term.print_blue("Lecture Donnees Communes : " + dossier_complet_file + " ...")
...
def load_artificialisation(dossier_artificialisation_file: str = dossierArtificialisationFile):
    ...
    Files.downloadFile(artificialisationSourceFile, dossierArtificialisationFile)
    if (dossierArtificialisation is None):
        Term.print_blue("Lecture Donnees Artificialisation : " + dossier_artificialisation_file + " ...")
...
dossierArtificialisationPaca : pd.DataFrame = None
...
def load_artificialisationPaca(dossier_artificialisationPaca_file: str = dossierArtificialisationPacaFile):
    ...
    if (dossierArtificialisationPaca is None):
        Term.print_blue("Lecture Donnees Artificialisation Paca : " + dossier_artificialisationPaca_file + " ...")
...
flux2019SourcePage = "https://www.insee.fr/fr/statistiques/6454108"
flux2019SourceFile = "https://www.insee.fr/fr/statistiques/fichier/6454108/base-csv-flux-mobilite-residentielle-2019.zip"
flux2019SourceMeta = "https://www.insee.fr/fr/statistiques/6454108#dictionnaire"
flux2019SourceData = data_dir + "base-flux-mobilite-residentielle-2019.csv"
flux2019 : pd.DataFrame = None


def load_flux_2019(p_flux2019SourceFile:  str = flux2019SourceData):
    global flux2019
    if (flux2019 is not None) : return flux2019
    Term.print_blue("Lecture Flux Mobilite Residentielle 2019 : " + p_flux2019SourceFile + " ...")
    flux2019 = pd.read_csv(p_flux2019SourceFile, delimiter=';',
...
flux2018 : pd.DataFrame = None


def load_flux_2018(p_flux2018SourceFile:  str = flux2018SourceData):
    global flux2018
    if (flux2018 is not None) : return flux2018
    Term.print_blue("Lecture Flux Mobilite Residentielle 2018 : " + p_flux2018SourceFile + " ...")
    flux2018 = pd.read_csv(p_flux2018SourceFile, delimiter=';',
...
flux2017 : pd.DataFrame = None


def load_flux_2017(p_flux2017SourceFile: str = flux2017SourceData):
    global flux2017
    if (flux2017 is not None) : return flux2017
    Term.print_blue("Lecture Flux Mobilite Residentielle 2017 : " + p_flux2017SourceFile + " ...")
    flux2017 = pd.read_csv(p_flux2017SourceFile, delimiter=';',
               dtype={'CODGEO': str, 'LIBGEO': str, 'DCRAN': str, 'L_DCRAN': str, 'NBFLUX_C17_POP01P' : float})
    return flux2017
...
flux2016 : pd.DataFrame = None


def load_flux_2016(flux2016SourceFile:  str = flux2016SourceFile):
    global flux2016
    if (flux2016 is not None) : return flux2016
    Term.print_blue("Lecture Flux Mobilite Residentielle 2016 : " + flux2016SourceFile + " ...")
    flux2016 = pd.read_csv(flux2016SourceFile, delimiter=';', encoding="ISO-8859-1",
               dtype={'CODGEO': str, 'LIBGEO': str, 'DCRAN': str, 'L_DCRAN': str, 'NBFLUX_C16_POP01P' : float})
    return flux2016


flux2015SourceFile = data_dir + "base-texte-flux-mobilite-residentielle-2015.csv"
flux2015 : pd.DataFrame = None


def load_flux_2015(flux2015SourceFile:  str = flux2015SourceFile):
    global flux2015
    if (flux2015 is not None) : return flux2015
    Term.print_blue("Lecture Flux Mobilite Residentielle 2015 : " + flux2015SourceFile + " ...")
    flux2015 = pd.read_csv(flux2015SourceFile, delimiter=';', encoding="ISO-8859-1",
               dtype={'CODGEO': str, 'LIBGEO': str, 'DCRAN': str, 'L_DCRAN': str, 'NBFLUX_C15_POP01P' : float})
    return flux2015


flux2014SourceFile = data_dir + "base-texte-flux-mobilite-residentielle-2014.csv"
flux2014 : pd.DataFrame = None


def load_flux_2014(flux2014SourceFile: str = flux2014SourceFile):
    global flux2014
    if (flux2014 is not None) : return flux2014
    Term.print_blue("Lecture Flux Mobilite Residentielle 2014 : " + flux2014SourceFile + " ...")
    flux2014 = pd.read_csv(flux2014SourceFile, delimiter=';', encoding="ISO-8859-1",
               dtype={'CODGEO': str, 'LIBGEO': str, 'DCRAN': str, 'L_DCRAN': str, 'NBFLUX_C14_POP01P' : float})
    return flux2014
...
fluxpro2018 : pd.DataFrame = None


# CODGEO;LIBGEO;DCLT;L_DCLT;NBFLUX_C18_ACTOCC15P
def load_fluxpro_2018(p_fluxpro2018SourceFile:  str = fluxpro2018SourceData):
    global fluxpro2018
    if (fluxpro2018 is not None) : return fluxpro2018
    Term.print_blue("Lecture Flux Mobilite Domicile Travail 2018 : " + p_fluxpro2018SourceFile + " ...")
...
fluxpro2019SourcePage = "https://www.insee.fr/fr/statistiques/6454112"
fluxpro2019SourceFile = "https://www.insee.fr/fr/statistiques/fichier/6454112/base-csv-flux-mobilite-domicile-lieu-travail-2019.zip"
fluxpro2019SourceMeta = "https://www.insee.fr/fr/statistiques/6454112#dictionnaire"
fluxpro2019SourceData = data_dir + "base-flux-mobilite-domicile-lieu-travail-2019.csv"
fluxpro2019 : pd.DataFrame = None


# CODGEO;LIBGEO;DCLT;L_DCLT;NBFLUX_C19_POP01P
def load_fluxpro_2019(p_fluxpro2019SourceFile:  str = fluxpro2019SourceData):
    global fluxpro2019
    if (fluxpro2019 is not None) : return fluxpro2019
    Term.print_blue("Lecture Flux Mobilite Domicile Travail 2019 : " + p_fluxpro2019SourceFile + " ...")
    fluxpro2019 = pd.read_csv(p_fluxpro2019SourceFile, delimiter=';',
                  dtype={'CODGEO': str, 'LIBGEO': str, 'DCRAN': str, 'L_DCRAN': str, 'NBFLUX_C19_POP01P' : float})
    return fluxpro2019
...
pop2019 : pd.DataFrame = None


# REG;DEP;COM;PMUN;PCAP;PTOT
def load_pop2019(p_pop2019SourceFile:  str = pop2019SourceData):
    global pop2019
    if (pop2019 is not None) : return pop2019
    Term.print_blue("Lecture Populations Légales 2019 : " + p_pop2019SourceFile + " ...")
    pop2019 = pd.read_csv(p_pop2019SourceFile, index_col=2, delimiter=';',
              dtype={'REG': str, 'DEP': str, 'COM': str, 'PMUN': int, 'PCAP' : int, 'PTOT' : int})
    return pop2019
...
stock2018 : pd.DataFrame = None


def load_stock_2018(p_stock2018SourceFile:  str = stock2018SourceData):
    global stock2018
    if (stock2018) : return stock2018
    Files.downloadFile(stock2018SourceFile, stock2018SourceData, is_zipped=True, zipped_file="FD_LOGEMTZE_2018.csv")
    Term.print_blue("Lecture Besoins de Logements en Stock 2018 : " + stock2018SourceFile + " ...")
    stock2018 = pd.read_csv(p_stock2018SourceFile, delimiter=';',
                dtype={'CODGEO': str, 'LIBGEO': str, 'DCRAN': str, 'L_DCRAN': str, 'NBFLUX_C18_POP01P' : float })
    return stock2018
...
collectDataMetrics  : pd.DataFrame = None
collectDiagnostics  : pd.DataFrame = None
collectCalculations : pd.DataFrame = None
SCOT_OUEST          : pd.DataFrame = None
...
def load_collectData(collect_file: str = collectDataFile):
    ...
    if (collectDataMetrics is None) :
        Term.print_blue("Lecture Data Metrics : " + collect_file + " ...")
        ...
        Term.print_yellow("  - Generation : " + "datametrics.json" + " ...")
        ...
        Term.print_yellow("  - Generation : " + "calculations.json" + " ...")
        ...
        Term.print_yellow("  - Generation : " + "diagnostics.json" + " ...")
...
class Files:
    ...
    def save_file(data, file_name: str):
        try:
            ...
            Term.print_red("Error Writing File : " + file_name + "-" + str(e))
            raise
    ...
    @staticmethod
    def downloadFile(url: str, filename: str, is_zipped=False, zipped_file: str = None) -> str:
        """ Download Files and Unzip"""
        if (os.path.isfile(filename)): return filename
        Term.print_red("Downloading : "+filename)
        ...
        with requests.get(url, stream=True) as r:
            ...
            print("")
        if (is_zipped):
            Term.print_red("Unzipping : " + filename)
            with zipfile.ZipFile(local_filename, 'r') as zip_ref:
                zip_ref.extractall("zip")
            shutil.move('zip' + os.sep + zipped_file, filename)
        Term.print_red("Extracted : " + local_filename)
        return local_filename

    @staticmethod
    def clean(logs : bool = True, pattern : str = "*", meta : bool = True):
        Term.print_red("##> Output Files Clean up.")
        Files.delete_pattern(output_dir, "*"+pattern+"*.png")
        Files.delete_pattern(output_dir, "*"+pattern+"*.csv")
        Files.delete_pattern(output_dir, "*"+pattern+"*_*.json")
        Files.delete_pattern(output_dir, "*"+pattern+"*.xlsx")
        Files.delete_pattern(output_dir, "*"+pattern+"*.html")
        if (logs):
            Files.delete_pattern(output_dir, "*.log")
        if (meta):
            Files.delete_pattern(output_dir, "context.yaml")
            Files.delete_pattern(output_dir, "france.json")
            Files.delete_pattern(output_dir, "datametrics.json")
            Files.delete_pattern(output_dir, "select.json")
            Files.delete_pattern(output_dir, "diagnostics.json")
...
class Term:
    ...
    def print_data(p_data_dict: dict):
        for l_key in p_data_dict.keys():
            Term.print_white(l_key + " : " + str(p_data_dict[l_key]))

    @staticmethod
    def print_data_frame(p_data_frame: pd.DataFrame, code_insee: str):
        for column in p_data_frame:
            Term.print_white(column + " - " + str(p_data_frame[column]["meta"]) + " : " + str(p_data_frame[column][code_insee]))

    @staticmethod
    def print_commune(commune : Union[int, list[int], list[str]], text : str = "") -> None:
        if (isinstance(commune, int)):
            Term.print_white(str(commune)+" : "+nom_commune(commune))
            return
        Term.print_blue(">> Communes : "+text)
        for comm in commune :
            Term.print_white(str(comm)+" : "+nom_commune(comm))
        return

    @staticmethod
    def print_epci(ecpi : Union[int, list[int], list[str]], text : str = "") -> None:
        if (isinstance(ecpi, int)):
            Term.print_white(str(ecpi)+" : "+nom_epci(ecpi))
            return
        Term.print_blue(">> Epcis : "+text)
        for ep in ecpi :
            Term.print_white(str(ep)+" : "+nom_epci(ep))
        return

    @staticmethod
    def print_dept(dept : Union[int, list[int], list[str]], text : str = "") -> None:
        if (isinstance(dept, int)):
            Term.print_white(str(dept)+" : "+nom_dept(dept))
            return
        Term.print_blue(">> Departements : "+text)
        for dp in dept :
            Term.print_white(str(dp)+" : "+nom_dept(dp))
        return
...
def f_percent(part, full, rounding=1, suffix="%", pformat=""):
    """ Retourne formatted part in percent of full     Part = 90, Full = 200 => 45 """
    # Adds suffix: suffix="%"   =>   45%
    # Adds format: pformat="+"   =>  +45%
    # Adds format: pformat="+()" =>  (+45%) 90
    # Adds format: pformat="()"  =>   (45%) 90
    percent = 0 if (float(full) == 0)  else (float(part) / float(full)) * 100
    s_percent = round0str(percent, rounding)
    if (pformat == ""):           return s_percent+suffix
    if (("+" in pformat) and (percent > 0)): s_percent = "+"+s_percent
    if ("(" in pformat): return "("+s_percent+suffix+") "+str(part)
    return "("+s_percent+suffix+") " + str(part)


def f_diff(after, before, pformat="+"):
    """  Adds pformat: format="+"   =>  +45% """
    # After = 200, Before = 100 => 100
    diff = after - before
    if (("+" in pformat) and (float(diff) > 0)): return ("+"+str(diff))
    return ""+str(diff)


def f_val(val, pformat="+", suffix=""):
    if (("+" in pformat) and (float(val) > 0)): return ("+"+str(val)+suffix)
    return ""+str(val)+suffix


def f_taux(value, rounding=2, suffix="%", pformat="+"):
    if ((float(value) > 0) and ("+" in pformat)): return ("+"+round0str(value, rounding)+suffix)
...
def get_code_insee_commune(code_postal) -> (int, str):
    ...
    if (not the_list):
        return 0, "Pas de Code INSEE pour code Postal "+str(code_postal)
...
def get_code_insee(code_postal : Union[int, str]) -> int:
    ...
    code_insee, nom_commune = get_code_insee_commune(code_postal)
    if str(nom_commune).startswith("Pas de"): return 0
    return code_insee
...
def nom_epci(code_epci, clean=False) -> Union[None, str]:
    """ Le nom de l'EPCI """
...
def code_epci(nom_epci : str) -> Union[int, str]:
    """ Le code de l'EPCI """
    load_interco()
...
def code_commune(nom_commune : str) -> Union[int, str]:
...
def nom_dept(code_dept, clean=False) -> str:
    ...
    if (code_dept == None) : return "Pas de Nom"
    if (code_dept == 0)    : return "Pas de Nom"
    if (code_dept == "")   : return "Pas de Nom"
    load_departements()
...
def code_departement(nom_dept) -> Union [int, str]:
    """ Le code du Departement """
    if (nom_dept == None) : return 0
    if (nom_dept == "")   : return 0
    load_departements()
...
def communes_zone(nom_zone) -> list[int]:
    ...
    scotData = scot_consolidation()
    if (nom_zone not in list_zones()) : return []
    return scotData["GROUPES_COMMUNES"][nom_zone]["COMMUNES"]


def dept_zone(nom_zone) :
    """ Le Code DEPT d'une Zone """
    scotData = scot_consolidation()
    if (nom_zone in scotData["GROUPES_COMMUNES"]) :
        return scotData["GROUPES_COMMUNES"][nom_zone]["DEPT"]
    else:
        return "99"
...
def nom_zone(key, clean=False) -> str :
    """ Le nom d'une Zone """
    scotData = scot_consolidation()
    if (key in scotData["GROUPES_COMMUNES"]) :
        nom = scotData["GROUPES_COMMUNES"][key]["NAME"]
        if (not clean): return nom
        return clean_name(nom)
    else:
        return "NZ"
...
def communes_territoire(territoire: str) -> [list[str], str]:
    ...
    elif code_departement(territoire):
        # Territoire = Nom Dept
        code_territoire = communes_dept(code_departement(territoire))
        type_territoire = "DEPT"
...
def list_dept(code_region=None) -> list[str]:
...
def get_sru2017(key, code_insee, rounding=6):
    """ Retourne la valeur consolidée de la cle SRU 17-19 pour la commune  """
    load_sru(sruFile)
    try:
...
def get_sru2020(key, code_insee, rounding=6):
    """ Retourne la valeur consolidée de la cle SRU 20-22 pour la commune  """
    load_sru(sruFile)
    try:
...
def get_art(key, code_insee, rounding=6):
    """ Retourne la valeur consolidée de la cle ART pour la commune  """
    load_artificialisation()
    try:
        return round0(dossierArtificialisation[key][code_insee], rounding)
    except Exception as e:
        Term.print_error(str(key)+" > Exception")
        Term.print_error(str(e))
        return 0


def get_art_ha(key, code_insee, rounding=6):
    """ Retourne la valeur consolidée de la cle ART pour la commune  en ha """
    load_artificialisation()
    try:
...
class DataStore():
    ...
    def add_metric(self, key : str, meta : str, source: str, mode : str, vtype: str, index : str = None, data=None, expr="None"):
        """ Add a metric to the data store """
        self.key_datas.append(key)
        self.meta_dict[key]    = meta
        self.source_dict[key]  = source
        self.type_dict[key]    = vtype
        self.mode_dict[key]    = mode
        self.expr_dict[key]    = expr
        self.error_dict[key]   = "OK"
        self.metric_list.append({"key" : key , "meta" : meta , "source" : source , "type" : vtype, "mode" : mode, "expr" : expr})
        return self.add_data(key, index, data)

    def add_diagnostic(self, _key: str, _description: str, test: str, messageV: str, data : bool, messageF: str, cat: str, vtype: str):
        """ Add a diagnostic to the data store """
        diagnostic = {"key"  : _key , "description" : _description , "test" : test, "value" : data, "categorie" : cat,
                      "type" : vtype,  "messageSiFaux" : messageF, "messageSiVrai" : messageV}
    ...
    def add_data(self, key : str, index : str, data):
        ...
        try:
            if (self.type_dict[key]   == "STR")     : return self.add_value(key, index, str(data))
                ...
                Term.print_red("else")
                Term.print_red(self.error_dict[key])
                Term.print_error("add_data Else : " + self.error_dict[key])
            ...
            Term.print_red("Exception")
            Term.print_red(self.error_dict[key])
            Term.print_error("add_data Exception : " + self.error_dict[key])
            self.add_value(key, index, str(data))
            # quit()
            return None
    ...
    def get_row_as_dict(self, index : str = None):
        """ Return the index raw as a dict with column names as keys.  """
        if (index is None): index = self.store_index
        if (index is None): return None
        dc = {}
    ...
    def save_data(self):
        ...
        Term.print_green("> Saving Data   : "+self.get_fullname())
        # DF TO JSON
        ## _s.json (Summary - used by GUI)
        Term.print_yellow("  - Saving Data : "+self.get_fullname() + "_s.json")
        ...
        with open(output_dir + self.get_fullname() + "_s.json", 'w') as f:
            ...
            for k in data_s["Data"].keys():
                Term.print_verbose(k)
                Term.print_verbose(str(data_s["Data"][k]))
        ...
        # DF TO EXCEL
        if (not FAST):
            Term.print_yellow("  - Saving Data : "+self.get_fullname() + ".xlsx")
            ...
            diag_df.to_excel(writer, "Diagnostics")
            # writer.save()
            writer.close()
        # DF TO CSV
        if (not FAST):
            Term.print_yellow("  - Saving Data : "+self.get_fullname() + ".csv")
        ...
        ## _m.json
        if (not FAST and FAST):
            Term.print_yellow("  - Saving Data : "+self.get_fullname() + "_m.json")
            all_data = dict()
    ...
    def load_data(self):
        ...
        Term.print_green("> Load Data : "+filename)
    ...
    def number(self, key, round=0, suffix="") -> str:
        """ Return an INT key value as a rounded number formatted string, with a suffix (e.g. %)  """
    ...
    def render_report(self, template=html_report_template, suffix=""):
        ...
        Term.print_green("> Render Report : " + html_file)
        ...
        Files.save_file(to_yaml(context), context_file)
        # Rendering Template
    ...
    # Get Data Frame without MetaData - Clean MetaData
    ...
    # Get Data Frame without Individual Data - Keep Total & MetaData
    def clean_data(self) :
    ...
    def collect_data(self, code_postal=None, code_insee=None):
        ...
        if (DataStoreCacheManager.load(code_insee, file=True)):
            Term.print_green("Cached Donnees : " + str(code_postal) + " : " + commune + " (Code INSEE : " + code_insee + ")")
            self.data_frame = DataStoreCacheManager.merge(self, code_insee)
            return self

        save_index       = self.store_index
        self.store_index = code_insee

        Term.print_green("> Collecte Donnees  : " + str(code_postal) + " : " + commune + " (Code INSEE : " + code_insee + ")")

        # Donnees Commune
        self.add_metric("CODE_INSEE",     "Code INSEE Commune", source=source_codes, mode=mode_count, data=code_insee, vtype="STR")
        self.add_metric("CODE_POSTAL",    "Code Postal Commune", source=source_codes, mode=mode_equal, data=code_postal, vtype="STR")
        self.add_metric("NOM_COMMUNE",    "Nom de Commune", source=source_codes, mode=mode_count, data=commune, vtype="STR")
        self.add_metric("LIBELLE",        "Libelle", source=source_codes, mode=mode_count, data=commune.title(), vtype="STR")
        self.add_metric("BASE_NAME",      "Nom Unique", source=source_codes, mode=mode_custom, data=self.get_fullname(), vtype="STR")

        # Donnees EPCI
        self.add_metric("EPCI",      "Code EPCI - Métropole", source=source_interco, mode=mode_count, data=intercoDossier["Unnamed: 2"][code_insee], vtype="STR")
        self.add_metric("LIBEPCI",   "Libellé de l'EPCI / Métropole", source=source_interco, mode=mode_count, data=intercoDossier["Unnamed: 3"][code_insee], vtype="STR")
        self.add_metric("TYPE_EPCI", "Nature d'EPCI", source=source_interco, mode=mode_count, data=intercoEPCI["Unnamed: 2"][self.get("EPCI")], vtype="STR")
        self.add_metric("EPCI_COMMUNES", "Nombre communes EPCI", source=source_interco, mode=mode_count, data=intercoEPCI["Unnamed: 3"][self.get("EPCI")], vtype="INT")
        self.add_metric("DEP",       "Departement", source=source_interco, mode=mode_count, data=intercoDossier["Unnamed: 4"][code_insee], vtype="STR")
        self.add_metric("DEP_NOM",   "Nom Departement", source=source_interco, mode=mode_count, data=departements["nom_departement"][self.get("DEP")], vtype="STR")
        self.add_metric("REG",       "Region", source=source_interco, mode=mode_count, data=intercoDossier["Unnamed: 5"][code_insee], vtype="STR")
        self.add_metric("REG_NOM",   "Nom Région", source=source_interco, mode=mode_count, data=departements["nom_region"][self.get("DEP")], vtype="STR")
        url_dossier = "https://www.insee.fr/fr/statistiques/2011101?geo=COM-" + self.str("CODE_INSEE")
        self.add_metric("DOSSIER_INSEE", "Dossier Complet INSEE", source=source_insee, mode=mode_custom, data=url_dossier, vtype="STR")
        global_context["URL_SOURCE_DOSSIER"] = url_dossier

        # Donnees Display
        self.add_metric("DISPLAY_TYPE",   "Type de Territoire", source=source_interco, mode=mode_count, data="Pas de Type", vtype="STR")
        self.add_metric("DISPLAY_COMM",   "Nom de Commune", source=source_interco, mode=mode_count, data="Pas de Nom", vtype="STR")
        self.add_metric("DISPLAY_EPCI",   "Nom de l'EPCI / Métropole", source=source_interco, mode=mode_count, data="Pas de Nom", vtype="STR")
        self.add_metric("DISPLAY_ZONE",   "Nom de la Zone", source=source_interco, mode=mode_count, data="Pas de Nom", vtype="STR")
        self.add_metric("DISPLAY_DEPT",   "Nom du Departement", source=source_interco, mode=mode_count, data="Pas de Nom", vtype="STR")
        self.add_metric("DISPLAY_REG",    "Nom de la Region", source=source_interco, mode=mode_count, data="Pas de Nom", vtype="STR")
        self.add_metric("DISPLAY_NAME",   "Nom du Territoire", source=source_interco, mode=mode_count, data="Pas de Nom", vtype="STR")
        ...
        com_sitadel17PL   = sitadel17PL.loc[sitadel17PL['COMM'] == str(code_insee)]
        com_sitadel       = pd.concat([com_sitadel1316, com_sitadel17PL])
        ...
        log_commences17PL = com_sitadel17PL.loc[com_sitadel17PL['Etat_DAU'] == "5"]
        log_termines17PL  = com_sitadel17PL.loc[com_sitadel17PL['Etat_DAU'] == "6"]
        ...
        com_sitadelLocaux2 = sitadel_locaux_17PL.loc[sitadel_locaux_17PL['COMM'] == str(code_insee)]
        ...
        for index, metric in collectDataMetrics.iterrows():
            ...
            value = 0
            try:
                Term.print_verbose("  - Evaluating Collect Line " + str(_line) + " :  [" + str(_key) + "] Expr : " + str(_expr))
                _data = re.sub("\${([A-Z0-9a-z-_]*)}", '\\1', _data)  # Replace ${VAR} by VAR
                value = eval(_data, self.get_row_as_dict(), {**globals(), **locals()})
                Term.print_verbose("  - Evaluating Collect Line " + str(_line) + " :  [" + str(_key) + "] Value : " + str(value) + " - " + str(type(value)))
                self.add_metric(str(_key), _description, source=_source, mode=_total, data=value, vtype=_type, expr=_expr)
            except Exception as e :
                error = "collect_data Error evaluating metrique : Line " + str(_line) + " Key = " + str(_key) + " expr : " + str(_expr) + " - Error : " + str(e)
                Term.print_error(error)
                if (_expr.startswith("error0")) : value = 0
                self.add_metric(str(_key), _description, source=_source, mode=_total, data=value, vtype=_type, expr=_expr)

        DataStoreCacheManager.update(self)
    ...
    def total_data(self, meta=True):
        ...
        Term.print_green("> Total Donnees   : " + str(self.store_code) + " : " + self.store_name)

        total_dict = {}
        _line = 0
        if ("__warningregistry__" in self.key_datas) : self.key_datas.remove("__warningregistry__")
        for key in self.key_datas:
            _line = _line + 1
            total_dict[key] = "ERROR Invalid Mode"
            Term.print_verbose("  - Evaluating Total Line " + str(_line) + ": [" + str(key) + "]")
            mode = self.mode_dict[key].strip()
            try:
                Term.print_verbose("  - Evaluating Total Line " + str(_line) + ": [" + str(key) + "] Mode : " + str(mode))
                ...
                Term.print_verbose("  - Evaluating Total Line " + str(_line) + ": [" + str(key) + "] Value : " + str(total_dict[key]))
            except Exception as e:
                Term.print_error("Exception Evaluating Total Line : [" + str(mode) + " ] for key : [" + str(key) + "] : " + str(e))
        ...
        for key in self.key_datas:
            ...
            if (key == "CODE_INSEE"): continue
                ...
                try:
                    Term.print_verbose("  - Evaluating Total Line : [" + str(key) + "] Mode : " + str(mode))
                    mode  = re.sub("\${([A-Z0-9a-z-_]*)}", '\\1', mode)    # Replace ${VAR} by VAR
                    value = eval(mode, total_dict, globals())
                    Term.print_verbose("  - Evaluating Total Line : [" + str(key) + "] Value : " + str(value) + " - " + str(type(value)))
    ...
    def run_calculs(self, meta=True, force=False):
        ...
        Term.print_green("> Calculs Donnees : " + str(self.store_code) + " : " + self.store_name)
        ...
        for index, metric in collectCalculations.iterrows():
            ...
            if (str(_key).startswith("#")) : continue   # Ignore line / key starting with #
            try:
                Term.print_verbose("  - Evaluating Calcul Line " + str(_line) + ": [" + str(_key) + "] Eval : " + str(_python))
                _python = re.sub("\${([A-Z0-9a-z-_]*)}", '\\1', _python)  # Replace ${VAR} by VAR
                value = eval(_python, self.get_row_as_dict(), {**globals(), **locals()})
                Term.print_verbose("  - Evaluating Calcul Line " + str(_line) + ": [" + str(_key) + "] Value : " + str(value) + " - " + str(type(value)))
                ...
                self.add_metric(str(_key), _description, source=_source, mode=_python, data=value, vtype=_type, expr=_python)
            except Exception as e :
                Term.print_error("Error evaluating Calcul : Line " + str(_line) + " Key = " + str(_key) + " + expr : " + str(_python) + " - Error : " + str(e))
                self.add_metric(str(_key), _description, source=_source, mode=_python, data=str(e), vtype=_type, expr=_python)
        self.store_index = save_index
        self.add_meta_data()
        self.calc_done = True
        return self
    ...
    def run_diagnostic(self, force=False):
        ...
        load_min_data()
        self.run_calculs()
        Term.print_green("> Établissement des diagnostics : " + str(self.store_code) + " : " + self.store_name)
        _line = 0
        for index, metric in collectDiagnostics.iterrows():
            ...
            value = None
            try:
                Term.print_verbose("  - Evaluating Diagnostic Line " + str(_line) + " : [" + str(_key) + "] Eval : " + str(_test))
                value = bool(eval(_test, self.get_row_as_dict(), {**globals(), **locals()}))
                Term.print_verbose("  - Evaluating Diagnostic Line " + str(_line) + " : [" + str(_key) + "] Value : " + str(value) + " - " + str(type(value)))
                ...
                self.add_diagnostic(str(_key), _description, test=_test, messageV=_messageV, data=value, messageF=_messageF, cat=_categorie, vtype=_type)
            except Exception as e :
                Term.print_error("Error evaluating Diagnostic : " + str(_key) + " + eval : " + _test + " - Error : " + str(e))
                self.add_diagnostic(str(_key), _description, test=_test, messageV=str(e), data=value, messageF=str(e), cat=_categorie, vtype=_type)
        self.diag_done = True
        return self
    ...
    def run_plots(self, force=False):
        ...
        Term.print_green("> Generation des Graphiques : " + str(self.store_code) + " : " + self.store_name)
        self.run_calculs()
        Plotter.plots(self)
        self.plot_done = True
        return self

    def report(self, force=True, ftp_push: bool = False):
        """ Generate report for DataStore. Force = True will re-calculate DataStore data from source. """
        load_min_data()
        Term.print_blue("Preparation Rapport "+self.store_type + " " + self.store_name + " (Code INSEE : "+self.store_code+")")
        loaded = None
        if (force is False) :
        ...
        if (not FAST and FAST):  # Disabling HTML Reports
            ...
            self.run_diagnostic()
            self.run_plots()
            HtmlReport.gen_tracker(self)
            html_report_file = HtmlReport.gen_report(ds=self)
        ...
        if (html_report_file):
            Term.display_in_browser(html_report_file)
...
class Flux:
    ...
    def report(year="all"):
        ...
        if (str(year) in ["all", "2018"]):
            load_flux_2018()
            Flux.report_details(flux2018, "NBFLUX_C18_POP01P" , "93", 'Flux2018-93.xlsx')
        if (str(year) in ["all", "2017"]):
            load_flux_2017()
            Flux.report_details(flux2017, "NBFLUX_C17_POP01P" , "93", 'Flux2017-93.xlsx')
        if (str(year) in ["all", "2016"]):
            load_flux_2016()
            Flux.report_details(flux2016, "NBFLUX_C16_POP01P" , "93", 'Flux2016-93.xlsx')
        if (str(year) in ["all", "2015"]):
            load_flux_2015()
            Flux.report_details(flux2015, "NBFLUX_C15_POP01P" , "93", 'Flux2015-93.xlsx')
        if (str(year) in ["all", "2014"]):
            load_flux_2014()
            Flux.report_details(flux2014, "NBFLUX_C14_POP01P" , "93", 'Flux2014-93.xlsx')

    @staticmethod
    def report_details(data_file, field, region, xls_file):
        # Donnees Flux
        Term.print_yellow("> Flux : "+xls_file)

        Term.print_green("- Flux - Communes")
        data1 = data_file.loc[(flux2019['CODGEO'].isin(communes_region(region)))  | (data_file['DCRAN'].isin(communes_region(region)))]

        Term.print_green("- Flux - EPCI")
        ...
        for x_epci in epci_region(region) :
            for y_epci in epci_region(region):
                xy_data = round(data_file.loc[flux2019['CODGEO'].isin(communes_epci(x_epci))  & data_file['DCRAN'].isin(communes_epci(y_epci)) & (data_file['DCRAN'] != data_file['CODGEO'])][field].sum(), 0)
        ...
        # Create Excel writer
        Term.print_green("- Flux - Saving File")
        ...
        # Save the Excel file
        writer.save()
        Term.print_yellow("< Flux : "+excel_file)


class DataStoreCacheManager:

    DataStoreCache = {}

    @staticmethod
    def update(ds : DataStore, code_insee=None):
        ...
        if code_insee and ((isinstance(code_insee, int)) or (isinstance(code_insee, str))):
            DataStoreCacheManager.DataStoreCache[str(code_insee)] = ds
            Term.print_green("> Added in Cache  : DataStore with code INSEE " + str(ds.store_code) + " : "  + ds.store_name)
        elif (isinstance(ds["CODE_INSEE"], int)) or (isinstance(ds["CODE_INSEE"], str)):
            DataStoreCacheManager.DataStoreCache[str(ds["CODE_INSEE"])] = ds
            Term.print_green("> Added in Cache  : DataStore with code INSEE " + str(ds["CODE_INSEE"]) + " : "  + ds.store_name)
        else:
            Term.print_red("- Not Added in Cache : DataStore without code INSEE : " + ds.store_name)

    @staticmethod
    def load(code_insee=None, file=True):
        """ Load a File for re-use without re-calculations """
        if (code_insee in DataStoreCacheManager.DataStoreCache) : return DataStoreCacheManager.DataStoreCache[code_insee]
        if (not file): return None
        global FORCE
        if (FORCE == True): return None
        ds = DataStore(nom_commune(code_insee), "COMMUNE", code_insee)
        if (ds.load_data() is None) : return None
        Term.print_green("> Loaded in Cache  : DataStore with code INSEE " + str(ds.store_code) + " : " + ds.store_name)
        DataStoreCacheManager.DataStoreCache[str(code_insee)] = ds
        return ds

    @staticmethod
    def merge(ds : DataStore, code_insee=None) -> pd.DataFrame :
        """ Update a DataStore from cache for Data with code_insee """
        l_data_frame = ds.data_frame.append(DataStoreCacheManager.DataStoreCache[code_insee].data_frame.loc[code_insee, :])
        # Add Meta Data
        if 'meta'   not in l_data_frame.index: l_data_frame = l_data_frame.append(DataStoreCacheManager.DataStoreCache[code_insee].data_frame.loc["meta", :])
        if 'mode'   not in l_data_frame.index: l_data_frame = l_data_frame.append(DataStoreCacheManager.DataStoreCache[code_insee].data_frame.loc["mode", :])
        if 'type'   not in l_data_frame.index: l_data_frame = l_data_frame.append(DataStoreCacheManager.DataStoreCache[code_insee].data_frame.loc["type", :])
        if 'source' not in l_data_frame.index: l_data_frame = l_data_frame.append(DataStoreCacheManager.DataStoreCache[code_insee].data_frame.loc["source", :])
        if 'expr'   not in l_data_frame.index: l_data_frame = l_data_frame.append(DataStoreCacheManager.DataStoreCache[code_insee].data_frame.loc["expr", :])
        ds.source_dict = DataStoreCacheManager.DataStoreCache[code_insee].source_dict
        ds.key_datas   = DataStoreCacheManager.DataStoreCache[code_insee].key_datas
        ds.meta_dict   = DataStoreCacheManager.DataStoreCache[code_insee].meta_dict
        ds.type_dict   = DataStoreCacheManager.DataStoreCache[code_insee].type_dict
        ds.source_dict = DataStoreCacheManager.DataStoreCache[code_insee].source_dict
        ds.mode_dict   = DataStoreCacheManager.DataStoreCache[code_insee].mode_dict
        ds.expr_dict   = DataStoreCacheManager.DataStoreCache[code_insee].expr_dict
        return l_data_frame
...
class HtmlReport:
    ...
    def gen_index(region="93"):
        ...
        return HtmlReport.render_index()
    ...
    def gen_report(ds : DataStore):
        """ Generates Report for DataStore """
        global_context["HTML_PLOT_LOGEMENTS"]          = Plotter.plot_logements(ds)
        global_context["HTML_PLOT_LOGEMENTS_PIE"]      = Plotter.plot_logements_pie(ds)
        global_context["HTML_PLOT_POPULATION"]         = Plotter.plot_population(ds)
        global_context["HTML_PLOT_TAILLE_DES_MENAGES"] = Plotter.plot_taille_menages(ds)
        global_context["HTML_SOURCES"]                 = HtmlReport.report_source(ds)
        global_context["HTML_DIAGNOSTIC"]              = HtmlReport.report_diagnostic(ds)
        global_context["HTML_TABLE_HISTORIQUE"]        = HtmlReport.report_historique(ds)
        global_context["HTML_TABLE_PROJECTIONS"]       = HtmlReport.report_projection(ds)
        global_context["HTML_TABLE_ARTIFICIALISATION"] = HtmlReport.report_artificialisation(ds)
        global_context["HTML_TABLE_SRU"]               = HtmlReport.report_sru(ds)
        global_context["HTML_TABLE_SUMMARY"]           = HtmlReport.report_summary_data(ds)
        global_context["HTML_FULL_DATA"]               = HtmlReport.report_full_data(ds)
        return ds.render_report()

    @staticmethod
    def readme_to_html():
        Term.print_green("> README.md to HTML     : " + "README.html")
...
class Plotter:
    ...
    @staticmethod
    def new_figure():
        Plotter.index_figure += 1
        plt.close('all')
        plt.figure(Plotter.index_figure)
        return plt

    @staticmethod
    def fig_to_base64(figure):
        ## Base64 figure for HTML Format
        img = io.BytesIO()
    ...
    def plots(ds: DataStore):
        ...
        for plot in desc["Plots"] :
            if (plot["Type"] == "LINES")  : Plotter.plots_lines(plot, ds)
            if (plot["Type"] == "PIE")    : Plotter.plots_pie(plot, ds)
            if (plot["Type"] == "DONUT")  : Plotter.plots_donut2(plot, ds)
            if (plot["Type"] == "DONUT2") : Plotter.plots_donut1(plot, ds)
    ...
    def plots_donut1(plot, ds: DataStore):
        ...
        ds.add_metric(key="GRAPHIQUE_"+key.upper(), meta=titre, source="PLOT", mode="EQUAL", vtype="STR", data=image_file_name, expr=image_file_name)
    ...
    def plots_lines(plot, ds: DataStore):
        ...
        for serie in plot["Series"] :
            ...
            if (serie["Smooth"] is True) :
                xsmooth, ysmooth = Plotter.plot_smooth(eval(serie["xValues"], ctx1, ctx2), eval(serie["yValues"], ctx1, ctx2))
            else:
    ...
    def plot_smooth(x_values : list[int], y_values : list[int]):
        ...
        xnew = np.linspace(x_values.min(initial=0), x_values.max(initial=0), 300)
    ...
    def plot_logements(ds: DataStore):
        """ Graphique des Logements en HTML et png. """
        data_dict = ds.get_row_as_dict(ds.store_index)
        label = "Logements"
        ...
        xsmooth, ysmooth = Plotter.plot_smooth([2008, 2013, 2018, 2020],
        ...
        xsmooth, ysmooth = Plotter.plot_smooth([2020, 2030],
                                       [data_dict["LOG_2020"] - data_dict["P08_RP"],
        ...
        xsmooth, ysmooth = Plotter.plot_smooth([2008, 2013, 2018],
        ...
        xsmooth, ysmooth = Plotter.plot_smooth([2018, 2020],
                                       [data_dict["P18_RP"] - data_dict["P08_RP"] + data_dict["P18_RSECOCC"] - data_dict["P08_RSECOCC"] + data_dict["P18_LOGVAC"] - data_dict["P08_LOGVAC"],
                                        data_dict["NOUV_LOG_0813"] + data_dict["NB_LGT_TOT_COMMENCES_1316"] + data_dict["NB_LGT_TOT_COMMENCES_17PL"]])
        plt.plot(xsmooth, ysmooth, color='#74248f', linestyle='dotted', linewidth=3, label="Indéterminées (Principales / Secondaires / Non-Vendues)")

        xsmooth, ysmooth = Plotter.plot_smooth([2013, 2016, 2020],
        ...
        xsmooth, ysmooth = Plotter.plot_smooth([2020, 2021],
                                       [data_dict["NOUV_LOG_0813"] + data_dict["NB_LGT_TOT_COMMENCES_1316"] + data_dict["NB_LGT_TOT_COMMENCES_17PL"],
        ...
        if ((data_dict["SRU_CARENCE_2020"] != 0) or (data_dict["NB_LGT_PRET_LOC_SOCIAL_1316"] + data_dict["NB_LGT_PRET_LOC_SOCIAL_17PL"]) != 0):
            xsmooth, ysmooth = Plotter.plot_smooth([2013, 2016, 2020],
                                           [data_dict["NOUV_LOG_0813"],
                                            data_dict["NOUV_LOG_0813"] + data_dict["NB_LGT_PRET_LOC_SOCIAL_1316"],  # Logements Sociaux Construits
                                            data_dict["NOUV_LOG_0813"] + data_dict["NB_LGT_PRET_LOC_SOCIAL_1316"] + data_dict["NB_LGT_PRET_LOC_SOCIAL_17PL"]])
        ...
        encoded_fig = Plotter.fig_to_base64(plt)
        ...
        browser_html = browser_html + "<p> - Logements Construits : " + str(round(ds["NB_LGT_TOT_COMMENCES_13PL"])) + "</p>"
        ...
        browser_html = browser_html + "<p> - Logements Construits : " + str(ds["NB_LGT_TOT_COMMENCES_13PL"])     + "</p>"
        browser_html = browser_html + "<p> - Logements Sociaux  : "   + str(ds["NB_LGT_PRET_LOC_SOCIAL_13PL"]) + "</p>"
        browser_html = browser_html + "<p> - Taux LS Construits : " + str(round(100 * ds["TX_LGT_PRET_LOC_SOCIAL_13PL"], 2)) + "% </p>"
    ...
    def plot_logements_pie(ds: DataStore):
        ...
        ## Save for HTML Format
        encoded_fig = Plotter.fig_to_base64(fig)
        browser_html = '<h2>'+label+'</h2><p><img src="data:image/png;base64, {}"></p>'.format(encoded_fig.decode('utf-8'))
        # email_html   = "<img alt=" + str(lKpi.get_data(c_name)) + " src=\"cid:" + image_file_name + "\" >"
    ...
    def plot_taille_menages(ds: DataStore):
        ...
        xsmooth, ysmooth = Plotter.plot_smooth([2020, 2030, 2040, 2050],
                                       [data_dict["TM_2020"],
                                        data_dict["TM_2030"],
                                        data_dict["TM_2040"],
                                        data_dict["TM_2050"]])
    ...
    def push_ds(ds : DataStore):
        ...
        if (not FAST):
            ...
            for png_file in [f for f in os.listdir(output_dir) if re.match(ds.get_fullname() + '.*\.png', f)] :
                file_list.append(output_dir + png_file)
        FTP.push_file(file_list)

    @staticmethod
    def push_file(filename, p_host="sobrietefonciere.fnepaca.fr", p_user="sobrietefonciere@fnepaca.fr", p_password="6TlHc!cjmd8u", p_directory="", prod=True):
        global FTP_PROD
        host = p_host
        user = p_user
        password = p_password
            ...
            try:
                ...
                for file in filename:
                    if (not os.path.exists(file)):
                        Term.print_red("File not found : " + file)
                        continue
                    Term.print_blue("FTP Push [" + host + "] : " + file)
                    fd = open(file, 'rb')
                    ftp.storbinary('STOR ' + file.replace("\\" , "/"), fd)
                    fd.close()
                ftp.close()
            except Exception as e:
                Term.print_red("FTP Upload Failed for  [" + host + "] : " + str(filename))
                Term.print_red(str(e))
    ...
    def push_files():
        ...
        if not FAST:
            filelist.extend([
                    ...
                    "input/mySophia-Logo.jpg",      "input/CartoPaca3-Light-Green.png",
                    ])
        FTP.push_file(filelist)
...
class Report:
    ...
    @staticmethod
    def commune(code_insee : str = None, code_postal: str = None, force=True, ftp_push : bool = False):
        ...
        if ((not commune) or (str(commune).startswith("Pas"))):
            Term.print_red("COMMUNE non trouvée pour code : INSEE ["+str(code_insee+"] POSTAL : ["+str(code_postal)+"]"))
            return None
    
        if (code_insee in communes_zone("SCoT_Ouest")):
            Term.print_blue("Scot Ouest pour Commune "+str(commune)+" : INSEE ["+str(code_insee+"]"))
            Report.scot_ouest_commune(code_insee=code_insee)
    
        if (commune.upper() == "ANTIBES"):
            Term.print_blue("Scot Ouest pour Commune "+str(commune)+" : INSEE ["+str(code_insee+"]"))
            Report.scot_ouest_commune(code_insee=code_insee, start_date="2018-01-01", file="scot_Antibes")
    
        entite = entite_commune
        name   = commune
        code   = code_insee
        return DataStore(store_name=name, store_type=entite, store_code=code).report(force=force, ftp_push=ftp_push)
    
    @staticmethod
    def zone(zone_name: str = "SCoT_Ouest_Littoral", force=True, with_communes=False, ftp_push : bool = False):
        ...
        if ((not name) or (str(name).startswith("Pas")) or (zone_name not in list_zones())):
            Term.print_red("Zone non trouvée pour code :  [" + str(zone_name) + "]")
            return None
        if (with_communes):
    ...
    @staticmethod
    def epci(epci_id: str = "200039915", force=True, with_communes=False, ftp_push : bool = False):
        """ Generates Report for an EPCI identified by Code INSEE  """
        entite = entite_epci
        name   = nom_epci(epci_id, clean=True)
        code   = epci_id
        if ((not name) or (str(name).startswith("Pas"))):
            Term.print_red("EPCI non trouvée pour code :  [" + str(epci_id) + "]")
            return None
        if (with_communes):
    ...
    @staticmethod
    def dept(dept_id: str = "06", force=True, with_communes=False, ftp_push : bool = False):
        """ Generates Report for a Departement identified by Code INSEE  """
        entite = entite_dept
        name   = nom_dept(dept_id, clean=True)
        code   = dept_id
        if ((not name) or (str(name).startswith("Pas"))):
            Term.print_red("DEPT non trouvé pour code :  [" + str(dept_id) + "]")
            return None
        if (with_communes):
    ...
    @staticmethod
    def region(reg_id: str = "93", force=True, with_communes=False, ftp_push : bool = False):
        """ Generates Report for a Region identified by Code INSEE  """
        entite = "REGION"
        name   = nom_region(reg_id, clean=True)
        code   = reg_id
        if ((not name) or (str(name).startswith("Pas"))):
            Term.print_red("REGION non trouvée for pour code :  [" + str(reg_id) + "]")
            return None
        if (with_communes):
            for dept in list_dept(reg_id):
                Report.dept(str(dept), force, with_communes=True, ftp_push=ftp_push)
            for commune in communes_region(reg_id):
                Report.commune(code_insee=str(commune), force=force, ftp_push=ftp_push)
        return DataStore(store_name=name, store_type=entite, store_code=code).report(force=force, ftp_push=ftp_push)
    
    @staticmethod
    def paca(force=True, ftp_push : bool = False):
        """ Generates Report for PACA Region  """
        Term.print_yellow("DEPT 06 - Alpes-Maritimes : ")
        for epci in epci_dept("06"):
            Report.epci(str(epci), force=force, ftp_push=ftp_push)
        Report.dept("06", force=force, ftp_push=ftp_push)
    
        Term.print_yellow("DEPT 04 - Alpes-de-Haute-Provence : ")
        for epci in epci_dept("04"):
            Report.epci(str(epci), force=force, ftp_push=ftp_push)
        Report.dept("04", force=force, ftp_push=ftp_push)
    
        Term.print_yellow("DEPT 05 - Hautes-Alpes : ")
        for epci in epci_dept("05"):
            Report.epci(str(epci), force=force, ftp_push=ftp_push)
        Report.dept("05", force=force, ftp_push=ftp_push)
    
        Term.print_yellow("DEPT 13 - Bouches-du-Rhone : ")
        for epci in epci_dept("13"):
            Report.epci(str(epci), force=force, ftp_push=ftp_push)
        Report.dept("13", force=force, ftp_push=ftp_push)
    
        Term.print_yellow("DEPT 83 - Var : ")
        for epci in epci_dept("83"):
            Report.epci(str(epci), force=force, ftp_push=ftp_push)
        Report.dept("83", force=force, ftp_push=ftp_push)
    
        Term.print_yellow("DEPT 84 - Vaucluse : ")
        for epci in epci_dept("84"):
            Report.epci(str(epci), force=force, ftp_push=ftp_push)
        Report.dept("84", force=force, ftp_push=ftp_push)
    
        for zone in list_zones():
            Report.zone(str(zone), force=force, ftp_push=ftp_push)
    
        Term.print_yellow("REGION 93 - Region PACA : ")
...
def report_select_dict(region=None, filename=None, force=False) -> dict:
    ...
    for r in lr:
        ...
        rd["Clean"]  = "REGION_" + nom_region(r, clean=True) + "_" + str(r)
        rd["Region"] = str(r)
        rd["DEPARTEMENTS"] = []
...
def report_region_dict(region=None, filename=None, force=False) -> dict:
    ...
    if ((force == False) and (os.path.isfile(filename))):
        Term.print_yellow("+ Reading  Index Region " + str(region)  + ": " + filename)
        with open(filename, "r") as read_file:
            Term.print_grey("Converting JSON encoded data into Python dictionary : " + filename)
...
class TestSobriete(unittest.TestCase):
    ...
    def runCommune(self, commune : str, report : bool = False) -> DataStore:
        global DISPLAY_HTML
        DISPLAY_HTML = True if (report) else False
        insee_commune = get_code_insee_commune_nom(commune)
        self.assertNotEqual(insee_commune, 0, msg="Commune inconnue")
        postal_commune, nom_commune =  get_code_postal_commune(insee_commune)
        self.assertNotEqual(postal_commune, 0, msg="Commune inconnue")
        Files.clean(logs=False, meta=False, pattern=str(insee_commune))
        Term.print_yellow("> " + str(nom_commune))
        ds = Report.commune(code_postal=str(postal_commune), force=True)
        Term.print_yellow("< " + str(nom_commune))
        self.assertEqual(ds.get("NOM_COMMUNE"), str(nom_commune).upper())
        return ds

    def testCannes(self):
        ds = self.runCommune("CANNES")
        self.assertEqual(ds.get("NOM_COMMUNE"), "CANNES")

    def testMougins(self):
        ds = self.runCommune("MOUGINS")
        self.assertEqual(ds.get("NOM_COMMUNE"), "CANNES")

    def testAntibes(self):
        ds = self.runCommune("Antibes")
        self.assertEqual(ds.get("NOM_COMMUNE"), "ANTIBES")

    def testSaintTropez(self):
        ds = self.runCommune("ST TROPEZ")
    ...
    def testZone(self):
        ...
        Term.print_yellow("> SCoT Ouest Littoral")
        ds  = Report.zone(zone_name="SCoT_Ouest_Littoral", force=True, with_communes=True)
        Term.print_yellow("< SCoT Ouest Littoral")
    ...
    def testSCoT_Ouest(self):
        global FAST
        FAST = True
        Term.print_yellow("> SCoT Ouest")
        ds = Report.zone(zone_name="SCoT_Ouest", force=True, with_communes=True)
        Term.print_yellow("< SCoT Ouest")
    ...
    def testCAPL(self):
        ...
        FAST    = True
        VERBOSE = True
        Term.print_yellow("> CA Cannes Pays de Lerins")
        ds = Report.epci(epci_id="200039915", force=True, with_communes=True)
    ...
    def testAlpesMaritimes(self):
        global FAST
        FAST = True
        Term.print_yellow("> Departement Alpes Maritimes")
        ds = Report.dept(dept_id="06", force=False, with_communes=True)
        Term.print_yellow("< Departement Alpes Maritimes")
        self.assertEqual(str(ds.get("NOM_COMMUNE")), "Alpes-Maritimes")

    # REGION Test
    def testPaca(self):
        Term.print_yellow("> Region Provence Alpes Cote d'Azur")
        ds = Report.region(reg_id="93", force=False)
        Term.print_yellow("< Region Provence Alpes Cote d'Azur")
    ...
    def testData(self):
        ...
        Term.print_yellow("> Nom EPCI 200039915")
        ...
        Term.print_yellow("> Nom Departement 06")
        ...
        Term.print_yellow("> Nom Commune 06250")
        ...
        print(str(lat))
        print(str(long))
        self.assertEqual("43.596083559", lat)
        self.assertEqual("7.001095284", long)
    ...
    def testArtificialisationData(self):
    ...
    def test_render_index(self):
        global DISPLAY_HTML
        DISPLAY_HTML = True
        html_index = HtmlReport.gen_index()
        Term.display_in_browser(html_index)

    def testPlots(self):
        ds = Report.commune(code_postal="06250", force=False)
        Plotter.plots(ds)
    ...
    def testArguments(self):
        arguments = "-f -a -p -s -d 06"
        read_command_line_args(str.split(arguments))
...
def read_command_line_args(argv):
    global DISPLAY_HTML, FORCE, FAST, VERBOSE
    global CONFIGURATION_FILE, TEMPLATE_FILE, FTP_PUSH, FTP_PROD
    ...
    usage = """
    Usage: -v -f -a -p -b -t -m -t -l -c <commune_code> -e <epci_code> -d <dept_code> -r <region_code> -z <zone_name> -o <start_date>    
           -l --list        : List all communes/epci/dept/zone in Territory       
           ...
           -p --push        : FTP Push Data to Alternatre Host WebSite (Production Sobriété Fonciere)         
           -s --prod        : FTP Push Data to Production Sobriété Fonciere         
           -f --force       : Reading source data (cache ignored)     
           -t --fast        : FasT (Only Data Refresh during Dev)     
           -m --meta        : Meta (Only Meta Refresh during Dev)     
           -v --verbose     : Verbose     
           -o --scot_ouest <d> : Report for Scot Ouest since start date 'd' ["arret", "executoire", "2020", "2021"]  
    ...
    try:
        opts, args = getopt.getopt(argv, "hanvbmpstflc:e:d:r:z:o:", ["help", "all", "verbose", "data", "fast", "push", "prod", "clean", "meta", "list", "commune=", "epci=", "dep=", "reg=", "zone=", "cxlsx=", "scot_ouest="])
    ...
    for opt, arg in opts:
        ...
        if optl in ("-h", "--help"):
            print(usage)
            quit()
        elif optl in ("-b", "--browse"):
            DISPLAY_HTML = True
            continue
        elif optl in ("-p", "--push", "-ftp"):
            FTP_PUSH = True
            FTP_PROD = False
            continue
        elif optl in ("-s", "-prod", "-sobriete"):
            FTP_PUSH = True
            FTP_PROD = True
            continue
        elif optl  in ("-v", "--verbose"):
            VERBOSE = True
            continue
        elif optl in ("-t", "--fast"):
            FAST = True
            continue
        elif optl in ("-f", "--force"):
            FORCE = True
            continue
        elif optl in ("-a", "--all"):
            WITH_COMMUNES = True
            ...
            Term.print_yellow("> Meta ")
            Report.push_meta()
            Term.print_yellow("< Meta ")
            quit()
        elif (optl == "--cxlsx"):
            if (not os.path.isfile(arg)):
                Term.print_red("Configuration File not found : "+arg)
                quit()
            ...
            if (not os.path.isfile(arg)):
                Term.print_red("Template File not found : "+arg)
                quit()
            TEMPLATE_FILE = arg
            continue
        elif optl in ("-l", "--list"):
            LIST_COMMUNE = True
            continue
        elif optl in ("-c", "--commune"):
            CODE_COMMUNE = arg
            Term.print_yellow("> Commune "+str(CODE_COMMUNE))
            Report.commune(code_insee=CODE_COMMUNE, force=FORCE, ftp_push=FTP_PUSH)
            Term.print_yellow("< Commune "+str(CODE_COMMUNE))
            quit()
        elif optl in ("-e", "--epci"):
            CODE_EPCI = arg
            setUp()
            Term.print_yellow("> EPCI " + str(CODE_EPCI))
            if (LIST_COMMUNE):
                Term.print_commune(communes_epci(CODE_EPCI))
            else:
                Report.epci(CODE_EPCI, force=FORCE, with_communes=WITH_COMMUNES, ftp_push=FTP_PUSH)
            Term.print_yellow("< EPCI " + str(CODE_EPCI))
            quit()
        elif optl in ("-z", "--zone"):
            ZONE_NAME = arg
            setUp()
            Term.print_yellow("> Zone " + str(ZONE_NAME))
            if (LIST_COMMUNE):
                Term.print_commune(communes_zone(ZONE_NAME))
            else:
                Report.zone(ZONE_NAME, force=FORCE, with_communes=WITH_COMMUNES, ftp_push=FTP_PUSH)
            Term.print_yellow("< Zone " + str(ZONE_NAME))
            quit()
        elif optl in ("-d", "--dept"):
            CODE_DEPT = arg
            setUp()
            Term.print_yellow("> Departement " + str(CODE_DEPT))
            if (LIST_COMMUNE):
                Term.print_blue(">> Communes ")
                Term.print_commune(communes_dept(CODE_DEPT))
                Term.print_blue(">> Epcis ")
                Term.print_epci(epci_dept(CODE_DEPT))
            else:
                Report.dept(CODE_DEPT, force=FORCE, with_communes=WITH_COMMUNES, ftp_push=FTP_PUSH)
            Term.print_yellow("< Departement " + str(CODE_DEPT))
            quit()
        elif optl in ("-o", "--scot_ouest"):
            start_date = arg
            setUp()
            Term.print_yellow("> Scot Ouest " + str(start_date))
            ...
            CODE_REGION = arg
            setUp()
            Term.print_yellow("> Region " + str(CODE_REGION))
            if (LIST_COMMUNE):
                Term.print_commune(communes_region(CODE_REGION))
                Term.print_epci(epci_region(CODE_REGION))
                Term.print_dept(list_dept(CODE_REGION))
            else:
                Report.region(CODE_REGION, force=FORCE, with_communes=WITH_COMMUNES, ftp_push=FTP_PUSH)
            Term.print_yellow("< Region " + str(CODE_REGION))
            quit()
...
###
# https://dillinger.io/
# https://sobrietefonciere.fnepaca.fr/README.md
